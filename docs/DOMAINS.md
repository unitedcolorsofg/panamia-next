...
**Until step 1 is done, `PANAVERSE_SUBDOMAINS` is `"0"` and Pana Social is reached at `/s` on whatever host is serving.** That flag gates links only, never routing: `originForFrom` returns the host in hand, so the surface switcher, `metadataBase` and every `canonical` stay on a hostname that resolves. It ships off because `social.pana.social` currently has no DNS record, and a link to a host with no record is a dead end rather than a slower route — the switcher pointed there for real, on the deployed site. `*.localhost` is exempt, so `social.localhost:3002` keeps exercising the subdomain path in dev while production does not.
...
2. Confirm `PANAVERSE_ROOT_DOMAIN=pana.social` in the deployed Worker vars, not only in `wrangler.jsonc`. **Then, and only then, set `PANAVERSE_SUBDOMAINS=1`** — this is the switch that moves the front door onto the subdomain. Setting it before step 1 resolves re-points every cross-surface link and canonical URL at a host that answers nothing, which is the state this flag exists to prevent. Verify by loading `https://social.pana.social/s` directly first; if that serves, the flag is safe to flip.
