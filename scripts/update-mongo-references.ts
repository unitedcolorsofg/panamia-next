...
async function main() {
  ...
  if (isDryRun) {
    ...
    console.log('⚠️  LIVE MODE - Changes WILL be written to MongoDB\n');
  }

  // Read ID mapping file
  console.log(`📖 Reading ID mapping file: ${mappingFilePath}`);
  ...
  try {
    ...
    async function updateCollection(
      ...
      console.log(
        `\n📦 ${collectionName}.${fieldPath}: Found ${documents.length} documents`
      );
      ...
      if (result.notMapped.length > 0) {
        console.log(
          `   ⚠️  ${result.notMapped.length} documents have unmapped user IDs`
        );
      }
      console.log(
    ...
    async function updateArrayField(
      ...
      console.log(
        `\n📦 ${collectionName}.${arrayField}[].${userIdField}: Found ${documents.length} documents`
      );
      ...
      if (result.notMapped.length > 0) {
        console.log(
          `   ⚠️  ${result.notMapped.length} unique unmapped user IDs in array`
        );
      }
      console.log(
        `   ${isDryRun ? 'Would update' : 'Updated'}: ${result.modified} documents`
    ...
    if (allNotMapped.length > 0) {
      console.log(
        `\n⚠️  ${allNotMapped.length} unique user IDs not in mapping:`
      );
      console.log(
    ...
    console.error('\n❌ Error updating MongoDB references:', error);
    process.exit(1);
  } finally {
    await client.close();
  }
