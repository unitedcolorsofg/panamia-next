-- Migration: 0025_events_nostr_model_v2
...
--
--          Supersedes 0024_events_nostr_model (now a no-op), which aborted with
--          42710 by re-adding host/venue FK names that 0017 already created.
--          Section 6 below drops the pre-existing FKs first.
-- Ticket: N/A
...
--
--    The host/venue FKs already exist, twice over: 0004 created
--    events_host_profile_id_fk / events_venue_id_fk, and 0017 added a second
--    pair under its own naming convention without dropping 0004's. Drop all
--    four and re-add a single clean pair under the 0017 convention, which is
--    what the Drizzle schema expects. (0024 omitted these drops and so aborted
--    with 42710.)
--
--    This also settles the ON DELETE conflict: 0017's host FK is SET NULL,
--    which contradicts the SET NOT NULL below. RESTRICT is the intended
--    behaviour -- a profile that still hosts events cannot be deleted.
ALTER TABLE "events" DROP CONSTRAINT IF EXISTS "events_host_profile_id_fk";
--> statement-breakpoint
ALTER TABLE "events" DROP CONSTRAINT IF EXISTS "events_host_profile_id_profiles_id_fk";
--> statement-breakpoint
ALTER TABLE "events" DROP CONSTRAINT IF EXISTS "events_venue_id_fk";
--> statement-breakpoint
ALTER TABLE "events" DROP CONSTRAINT IF EXISTS "events_venue_id_venues_id_fk";
--> statement-breakpoint
